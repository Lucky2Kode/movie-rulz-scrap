...
ALL_PAGES = -1  # sentinel for "scrape until empty"
MAX_RETRIES = 2  # retries on fetch error before skipping a page


def run(base_url: str, start: int = 1, end: int = 1) -> list[dict]:
    """
    Scrape a featured/category listing page.
    Pass end=ALL_PAGES (-1) to scrape all pages until none are left.

    Distinguishes between:
      - fetch error (None)  → retries up to MAX_RETRIES, then skips page
      - empty page ([])     → real end of pagination, stops
    """
    all_rows = []
    page = start
    ...
    while True:
        ...
        print(f"--- [featured] Page {page}: {url}")
